...
template<int StateLIdx = EKFState::StateDefinition_T::L, int StateQicIdx = EKFState::StateDefinition_T::q_ic, int StatePicIdx = EKFState::StateDefinition_T::p_ic, int StateQwvIdx = EKFState::StateDefinition_T::q_wv, int StatePwvIdx = EKFState::StateDefinition_T::p_wv>
struct PoseMeasurement : public PoseMeasurementBase {
 ...
 public:
  ...
  PoseMeasurement(double n_zp, double n_zq, bool measurement_world_sensor, bool fixed_covariance, bool isabsoluteMeasurement, int sensorID, int fixedstates, msf_updates::PoseDistorter::Ptr distorter = msf_updates::PoseDistorter::Ptr()) : PoseMeasurementBase(isabsoluteMeasurement, sensorID), n_zp_(n_zp), n_zq_(n_zq), measurement_world_sensor_(measurement_world_sensor), fixed_covariance_(fixed_covariance), distorter_(distorter), fixedstates_(fixedstates) {}
  ...
  virtual void CalculateH(shared_ptr<EKFState_T> state_in, Eigen::Matrix<double, nMeasurements, msf_core::MSF_Core<EKFState_T>::nErrorStatesAtCompileTime>& H) {
    ...
    Eigen::Matrix<double, 3, 3> C_ci = state.Get<StateQicIdx>().conjugate().toRotationMatrix();

    // Preprocess for elements in H matrix.
    Eigen::Matrix<double, 3, 1> vecold;

    vecold = (-state.Get<StatePwvIdx>() + state.Get<StateDefinition_T::p>() + C_q * state.Get<StatePicIdx>()) * state.Get<StateLIdx>();
    ...
    enum {
      kIdxstartcorr_p = msf_tmp::GetStartIndexInCorrection<StateSequence_T, StateDefinition_T::p>::value,
      kIdxstartcorr_v = msf_tmp::GetStartIndexInCorrection<StateSequence_T, StateDefinition_T::v>::value,
      kIdxstartcorr_q = msf_tmp::GetStartIndexInCorrection<StateSequence_T, StateDefinition_T::q>::value,

      kIdxstartcorr_L = msf_tmp::GetStartIndexInCorrection<StateSequence_T, StateLIdx>::value,
      kIdxstartcorr_qwv = msf_tmp::GetStartIndexInCorrection<StateSequence_T, StateQwvIdx>::value,
      kIdxstartcorr_pwv = msf_tmp::GetStartIndexInCorrection<StateSequence_T, StatePwvIdx>::value,
      kIdxstartcorr_qic = msf_tmp::GetStartIndexInCorrection<StateSequence_T, StateQicIdx>::value,
      kIdxstartcorr_pic = msf_tmp::GetStartIndexInCorrection<StateSequence_T, StatePicIdx>::value,
    };
    ...
    // Construct H matrix.
    // Position:
    H.block<3, 3>(0, kIdxstartcorr_p) = C_wv * state.Get<StateLIdx>()(0);  // p

    H.block<3, 3>(0, kIdxstartcorr_q) = -C_wv * C_q * pci_sk * state.Get<StateLIdx>()(0);  // q

    H.block<3, 1>(0, kIdxstartcorr_L) = scalefix ? Eigen::Matrix<double, 3, 1>::Zero() : (C_wv * C_q * state.Get<StatePicIdx>() + C_wv * (-state.Get<StatePwvIdx>() + state.Get<StateDefinition_T::p>())).eval();  // L

    H.block<3, 3>(0, kIdxstartcorr_qwv) = driftwvattfix ? Eigen::Matrix<double, 3, 3>::Zero() : (-C_wv * skewold).eval();  // q_wv

    H.block<3, 3>(0, kIdxstartcorr_pic) = calibposfix ? Eigen::Matrix<double, 3, 3>::Zero() : (C_wv * C_q * state.Get<StateLIdx>()(0)).eval();  //p_ic


    // TODO (slynen): Check scale commenting
    H.block<3, 3>(0, kIdxstartcorr_pwv) = driftwvposfix ? Eigen::Matrix<double, 3, 3>::Zero() : (-Eigen::Matrix<double, 3, 3>::Identity()
            /* * state.Get<StateLIdx>()(0)*/).eval();  //p_wv

    // Attitude.
    H.block<3, 3>(3, kIdxstartcorr_q) = C_ci;  // q

    H.block<3, 3>(3, kIdxstartcorr_qwv) = driftwvattfix ? Eigen::Matrix<double, 3, 3>::Zero() : (C_ci * C_q.transpose()).eval();  // q_wv

    H.block<3, 3>(3, kIdxstartcorr_qic) = calibattfix ? Eigen::Matrix<double, 3, 3>::Zero() : Eigen::Matrix<double, 3, 3>::Identity().eval();  //q_ic
  ...
  virtual void Apply(shared_ptr<EKFState_T> state_nonconst_new, msf_core::MSF_Core<EKFState_T>& core) {
    ...
    if (isabsolute_) {  // Does this measurement refer to an absolute measurement,
      ...
      // Construct residuals.
      // Position.
      r_old.block<3, 1>(0, 0) = z_p_ - (C_wv.transpose() * (-state.Get<StatePwvIdx>() + state.Get<StateDefinition_T::p>() + C_q.transpose() * state.Get<StatePicIdx>())) * state.Get<StateLIdx>();

      // Attitude.
      Eigen::Quaternion<double> q_err;
      q_err = (state.Get<StateQwvIdx>() * state.Get<StateDefinition_T::q>() * state.Get<StateQicIdx>()).conjugate() * z_q_;
      ...
      // Construct residuals.
      // Position:
      Eigen::Matrix<double, 3, 1> diffprobpos = (C_wv_new.transpose() * (-state_new.Get<StatePwvIdx>() + state_new.Get<StateDefinition_T::p>() + C_q_new.transpose() * state_new.Get<StatePicIdx>())) * state_new.Get<StateLIdx>() - (C_wv_old.transpose() * (-state_old.Get<StatePwvIdx>() + state_old.Get<StateDefinition_T::p>() + C_q_old.transpose() * state_old.Get<StatePicIdx>())) * state_old.Get<StateLIdx>();
      ...
      // Attitude:
      Eigen::Quaternion<double> diffprobatt = (state_new.Get<StateQwvIdx>() * state_new.Get<StateDefinition_T::q>() * state_new.Get<StateQicIdx>()).conjugate() * (state_old.Get<StateQwvIdx>() * state_old.Get<StateDefinition_T::q>() * state_old.Get<StateQicIdx>());
